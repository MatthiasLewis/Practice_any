...
for i in range(0,8):
    ...
    with open (f"sum_{file[i]['category']}_{file[i]['sum_title'][0:4]}","w",encoding="utf8") as f1:
    ...
    for s in range(0,3):
        ...
        soup = BeautifulSoup(res,"html.parser")
        content_all = soup.find("div",class_="indent").find_all("p")
        with open (f"spot_{file[i]['category']}_{spot_con[s]['title'][0:4]}","w",encoding="utf8") as f1:
          for p in content_all:
            json.dump(p.text,f1,ensure_ascii=False)
            f1.write("\n")
        time.sleep(4)
